...
수집과 파싱 로직은 KBO 일정 데이터를 다루는 범용 로직으로 유지하고, CSV 출력만 `re-seat` 본체 ERD/import 형식에 맞춘 어댑터로 분리합니다. 다른 프로젝트에서 재사용할 때는 `ReSeatCsvExporter`와 `ReSeatCsvSchema`만 필요한 출력 형식에 맞게 바꾸면 됩니다.
...
```text
id,name,address,total_capacity,status
```
...
```text
id,name,home_stadium_id,status
```
...
```text
id,home_team_id,away_team_id,stadium_id,game_at,booking_open_at,booking_close_at,booking_status,title
```

`game_key`는 스크래퍼 내부 DB에서 재수집/upsert용으로만 사용하고, `re-seat` import용 CSV에는 출력하지 않습니다.
...
6. `ReSeatCsvExporter`가 `re-seat` 본체 ERD 형식에 맞춰 `stadiums.csv`, `teams.csv`, `games.csv`를 생성합니다.
